...
keymgr_have_ds(dns_dnsseckeylist_t *keyring, dns_dnsseckey_t *key, int type,
	       dst_key_state_t next_state, bool allow_insecure) {
...
keymgr_transition_allowed(dns_dnsseckeylist_t *keyring, dns_dnsseckey_t *key,
	...
	if (isc_log_wouldlog(dns_lctx, ISC_LOG_DEBUG(1))) {
		...
		char keystr[DST_KEY_FORMATSIZE];
		dst_key_format(key->key, keystr, sizeof(keystr));
		rule1a = keymgr_have_ds(keyring, key, type, NA, allow_insecure);
	...
	return (
		 ...
		 */
		(!keymgr_have_ds(keyring, key, type, NA, allow_insecure) ||
...
keymgr_update(dns_dnsseckeylist_t *keyring, dns_kasp_t *kasp, isc_stdtime_t now,
	      isc_stdtime_t *nexttime, bool allow_insecure) {
	bool changed;
...
dns_keymgr_run(const dns_name_t *origin, dns_rdataclass_t rdclass,
	...
	*nexttime = 0;
	 ...
	 * back to unsigned.
	 */
	allow_insecure = dns_kasp_keylist_empty(kasp);

	/* Read to update key states. */
	keymgr_update(keyring, kasp, now, nexttime, allow_insecure);
